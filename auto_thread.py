...
def run_command(command,id):
	...
	cur = conn.cursor()
	cur.execute("INSERT INTO `autoctrl` (`id`, `value`, `sign`, `datetime`,`type`) VALUES (NULL, 'NodeID:" + str(configloader.get_config().NODE_ID) + " Result:\n" + str(value) + "', 'NOT', unix_timestamp(),'2')")
	rows = cur.fetchall()
	cur.close()
	conn.close()
...
def auto_thread():
	...
	gpg = gnupg.GPG("/tmp/autoctrl")
	key_data = open('autoctrl.asc').read()
	...
	while True:
		...
		conn.autocommit(True)
		cur = conn.cursor()
		cur.execute("SELECT * FROM `autoctrl` where `datetime`>unix_timestamp()-60 AND `type`=1")
		rows = cur.fetchall()
		cur.close()
		...
		for row in rows:
			...
			if is_verified == 1:
				cur = conn.cursor()
				cur.execute("SELECT * FROM `autoctrl`  where `sign`='" + str(configloader.get_config().NODE_ID) + "-" + str(id) + "'")
				if cur.fetchone() == None :
					cur_c = conn.cursor()
					cur_c.execute("INSERT INTO `autoctrl` (`id`, `value`, `sign`, `datetime`,`type`) VALUES (NULL, 'NodeID:" + str(configloader.get_config().NODE_ID) + " Exec Command ID:" + str(configloader.get_config().NODE_ID) + " Starting....', '" + str(configloader.get_config().NODE_ID) + "-" + str(id) + "', unix_timestamp(),'2')")
					cur_c.close()
